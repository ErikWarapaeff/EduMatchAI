...
def main():
    ...
    with tab1:
        ...
        if st.button("Найти схожий курс от GeekBrains"):
            ...
            st.markdown(
                f'<a href="{"https://raw.githubusercontent.com/ikanam-ai/VacancySkillsEstimator/main/logo.jpg"}"><img src="{"https://raw.githubusercontent.com/ikanam-ai/VacancySkillsEstimator/main/logo.jpg"}" alt="Foo" width="500" height="250"/></a>',
                unsafe_allow_html=True,
            )
            st.caption("**GeekBrains — лидер в сфере онлайн-образования**")
            ...
            st.markdown(
                f"**Больше информации:** [{data.loc[0]['url']}]({data.loc[0]['url']})"
            )
            ...
            def display_course_info(column, course_data):
                with column:
                    ...
                    st.markdown(
                        f'<a href="{course_data["url"]}"><img src="{example}" alt="Foo" width="150" height="150"/></a>',
                        unsafe_allow_html=True,
                    )
                    st.markdown(f"**{course_data['name']}**")
                    score = course_data["total_score"]
                    percent = (
                        100 if score <= 0 else round(100 * (score / (score + 1)), 2)
                    )
                    st.markdown(
                        f"<span style='color:green'>Схожесть: {percent}%</span>",
                        unsafe_allow_html=True,
                    )
    ...
    with tab2:
        ...
        if st.button("Найти схожий курс от GeekBrains "):
            ...
            skills2 = list(
                set(
                    [k.lower() for k in found_skills]
                    + [k.lower() for k in data.loc[0]["stack"]]
                )
            )
            ...
            st.caption("**GeekBrains — лидер в сфере онлайн-образовани**")
            st.markdown(f"**Стек:** {' , '.join(data.loc[0]['stack'])}")
            ...
            def display_course_info(column, course_data):
                with column:
                    ...
                    skills2 = list(
                        set(
                            [k.lower() for k in found_skills]
                            + [k.lower() for k in course_data["stack"]]
                        )
                    )
                    difference = compare_models(skills1, skills2)
                    st.markdown(
