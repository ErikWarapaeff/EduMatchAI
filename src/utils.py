...
def preprocess_and_match_vacancy(
    vacancy_url: str, data_gb_path: str, data_it_path: str
):
    ...
    data_gb["levenshtein_distance"] = data_gb.apply(
        levenshtein_distance_sort, args=(preprocessed_vacancy["name"],), axis=1
    )
    ...
    data_gb["levenshtein_distance_stack"], data_gb["max_intersection_stack"] = zip(
        *data_gb.apply(calculate_skill_similarity, args=(it_skills,), axis=1)
    )
    ...
    metrics_to_normalize = [
        ...
        "levenshtein_distance_stack",
        "max_intersection_stack",
        "tfidf_description",
    ]
    data_gb["total_score"] = scaler.fit_transform(data_gb[metrics_to_normalize]).sum(
        axis=1
    )
    data_gb = data_gb.sort_values(by="total_score", ascending=False).reset_index(
        drop=True
    )
...
def get_matrix_df(vacancy_url: str, data_gb_path: str):
    ...
    matrix_df = pd.DataFrame(
        index=preprocessed_vacancy["requirements_predict"], columns=data_gb.index
    )
