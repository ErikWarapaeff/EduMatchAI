...
- Рост трафика на платформу за счёт новых пользователей (целевой показатель — прирост >= 15%).  
...
- Монетизация платформы через привлечение рекламодателей.  

#### Бизнес-ограничения (Product Owner):  
- Ограничение на бюджет MVP.  
- Максимальная скорость обработки запросов (не более 3 секунд).  
...
- Пользователь вводит текст вакансии → система анализирует требования → выдаёт список подходящих курсов с платформы GeekBrains.  
