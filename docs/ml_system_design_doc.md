...
## Содержание
1. ## [Термины и пояснения](#термины-и-пояснения)
2. ## [Цели и предпосылки](#1-цели-и-предпосылки)
   ### 0.1. [Зачем идем в разработку продукта?](#11-зачем-идем-в-разработку-продукта)
   ### 0.2. [Бизнес-требования и ограничения](#12-бизнес-требования-и-ограничения)
   ### 0.3. [Что входит в скоуп проекта/итерации, что не входит](#13-что-входит-в-скоуп-проектаитерации-что-не-входит)
   ### 0.4. [Предпосылки решения](#14-предпосылки-решения)
3. ## [Методология](#2-методология)
   ### 0.6. [Постановка задачи](#21-постановка-задачи)
   ### 0.7. [Блок-схема решения](#22-блок-схема-решения)
4. ## [Этап 1.EDA и подготовка данных](#этап-1-eda-и-подготовка-данных)
   ### 1.2. [Классификация текста на наличие скиллов](#этап-12-классификация-текста-на-наличие-скиллов)
   ### 1.3. [Обработка вакансий и предсказание требований](#этап-13-обработка-вакансий-и-предсказание-требований)
   ### 1.4. [Обучение модели для предсказания увеличения заработной платы после прохождения курса](#этап-14-обучение-модели-для-предсказания-увеличения-заработной-платы-после-прохождения-курса)
5. ## [Этап 2. Подготовка прогнозных моделей](#этап-2-подготовка-прогнозных-моделей)
   ### 2.1. [Постановка задачи](#этап-21-постановка-задачи)
   ### 2.2. [Подготовка модели классификации](#подготовка-модели-классификации)
   ### 2.3. [Итог обучения](#итог-обучения)
   ### 2.4. [Подготовка и обучение для модели регрессии](#далее-подготовка-и-обучение-для-модели-регрессии)
6. ## [Этап 3.Подготовка пилота](#3-подготовка-пилота)
7. ## [Этап 4. Архитектура решения](#41-архитектура-решения)
...
# Этап 2.1: EDA и подготовка данных
...
На текущем этапе система представляет собой самостоятельный модуль на Python с использованием фреймворка Streamlit.  

**Назначение:**  
- Анализ вакансий и сопоставление их с образовательными программами  
- Рекомендация оптимальных курсов для повышения квалификации  

**Методы API:**  
- `preprocess_and_match_vacancy`: обработка вакансий и сопоставление данных  
- `get_matrix_df`: генерация матриц для анализа навыков  
- `ResumeSkillsFinder`: поиск навыков в резюме пользователя  

При необходимости интеграции с backend-системой может быть разработана блок-схема архитектуры.  

## 4.2. Описание инфраструктуры и масштабируемости  

**Инфраструктура:**  
- Локальное исполнение через Streamlit  
- Использование CSV-файлов для хранения данных образовательных программ  

**Плюсы:**  
- Быстрая развертка для демонстрации  
- Простота управления и внесения изменений  

**Минусы:**  
- Ограниченность ресурсов  
- Сложности с масштабируемостью  

**Выбор инфраструктуры объясняется:**  
- Простотой реализации для демонстрации MVP (Minimal Viable Product)  

## 4.3. Требования к работе системы  

- **SLA:** Отсутствие строгих требований на данном этапе  
- **Пропускная способность:** Обработка одной вакансии занимает 10–20 секунды.
- **Задержка:** Основные задержки связаны с загрузкой данных через API и NLP-обработкой  

## 4.4. Безопасность системы  

- **Потенциальные уязвимости:**  
  - Загрузка непроверенных ссылок или файлов, которые могут содержать вредоносный код  
...
- **Соответствие GDPR:**  
  - Обрабатываются только анонимные данные  
  - Персональные данные не собираются и не хранятся  

#### 4.6. Издержки  

- На текущем этапе издержки минимальны и ограничиваются вычислительными ресурсами локальной машины разработчика.  

## 4.7. Integration points  

Интеграция с backend-системой может быть реализована через REST API или gRPC. На текущем этапе взаимодействие ограничивается загрузкой URL и обработкой локальных данных.  

## 4.8. Риски  

- **Ограниченные ресурсы:** Сложности с масштабированием из-за локального исполнения на данном этапе  
- **Доступность данных:** Использование устаревших CSV-файлов может приводить к некорректным рекомендациям, планируется автоматизация процесса парсинга

## 4.9. Ограничения текущей версии  

На данный момент парсинг резюме возможен только по открытым ссылкам. Это означает:  
- Для корректной работы системы резюме должно быть доступно для всех пользователей.  
- Пользователю необходимо заранее сделать своё резюме открытым на платформе (например, на сайте или в банке резюме).  

Это ограничение связано с текущим подходом к обработке данных и будет пересмотрено в дальнейших обновлениях системы.  


# Масштабирование и дальнейшие планы

1. **Использование более быстрых языков программирования**  
   Для повышения производительности критически важных частей алгоритма планируется переход на более быстрые языки, такие как C++ или Rust. Это позволит:  
   - Ускорить обработку данных  
   - Снизить время отклика системы  
   - Увеличить объём данных, обрабатываемых в реальном времени  

2. **Развёртывание на сервере**  
   Для интеграции в производственные процессы и обработки больших объёмов данных алгоритм будет развернут на серверной инфраструктуре. Это обеспечит:  
   - Масштабируемость системы для промышленного использования  
   - Стабильность работы под нагрузкой  
   - Простоту доступа через веб-интерфейсы или API  
